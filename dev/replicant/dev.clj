    ...
    (for [{:keys [title k]} examples]
      [:li
       [:a.link
        ...
        title]])
    [:li [:a.link {:href "/cljs/"} "Client examples"]]]])

(defn render-example [id]
  (let [render (:f (first (filter (comp #{id} :k) examples)))]
    [:div#example {:data-example-id id}
     (render @store id)
     [:p [:a {:href "/"} "Back to examples"]]]))
...
(defn render-client-index []
  [:div
   [:div {:id "app"}]
   [:p [:a.link {:href "/"} "Server examples"]]])
...
(def layout
  (let [html (slurp (io/resource "public/index.html"))
        [pre post] (str/split html #"<div id=\"app\"></div>")]
    [pre
     (str "<script src=\"/server-actions.js\"></script>" post)]))
