...
(deftest has-bad-conditional-attrs?-test
  ...
  (testing "No attrs => attr map, with text"
    ;; This would be bad if it was indeed the same node, but it is likely not
    ;; generated by the same code since there's no explicit nil, and we don't
    ;; want to trigger this warning for different nodes that just ended up in
    ;; the same position.
    (is (false? (asserts/has-bad-conditional-attrs?
                (->vdom [:h1 "Hello"])
                (->headers [:h1 {} "Hi"])))))
  ...
  (testing "No attr nil => map, with text"
    (is (true? (asserts/has-bad-conditional-attrs?
                (->vdom [:h1 nil "Hello"])
                (->headers [:h1 {} "Hi"])))))
  ...
  (testing "No attr nil => map, with children"
    (is (true? (asserts/has-bad-conditional-attrs?
                (->vdom [:h1 nil [:span "Hello"]])
                (->headers [:h1 {} [:span "Hi"]])))))
  ...
  (testing "No attr nil => map, with list of children"
    (is (true? (asserts/has-bad-conditional-attrs?
                (->vdom [:h1 nil (list [:span "Hello"])])
                (->headers [:h1 {} (list [:span "Hi"])])))))
  ...
  (testing "Does not flag the warning for different elements that ended up in the same position"
    (is (false? (asserts/has-bad-conditional-attrs?
                 (->vdom [:h1 {} (list [:span "Hello"])])
                 (->headers [:h2 "Hi"]))))))
