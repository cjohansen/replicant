...
(testing "Re-creates unkeyed moved nodes"
  ...
  (testing "Does not re-add child nodes that did not move"
    (is (= (-> (h/render [:ul
                          ...
                          [:li {:rkey "5"} "Item #6"]])
               (h/render [:ul
                          [:li {:rkey "0"} "Item #1"] ;; Same pos
                          [:li {:rkey "4"} "Item #5"]
                          [:li {:rkey "2"} "Item #3"] ;; Same pos
                          [:li {:rkey "1"} "Item #2"]
                          [:li {:rkey "3"} "Item #4"]
                          [:li {:rkey "5"} "Item #6"] ;; Same pos
                          ])
               h/get-mutation-log-events
               h/summarize)
  ...
  (testing "Swaps nodes and adjusts attributes"
    (is (= (-> (h/render [:ul
                          [:li {:rkey "0" :title "Numero uno"} "Item #1"]
                          [:li {:rkey "1" :title "Numero dos"} "Item #2"]])
               (h/render [:ul
                          [:li {:rkey "1" :title "Number two"} "Item #2"]
                          [:li {:rkey "0" :title "Number one"} "Item #1"]])
               h/get-mutation-log-events
               h/summarize)
  ...
  (testing "Adds node in the middle of existing nodes"
    (is (= (-> (h/render [:div
                          [:h1 {} "Title"]
                          [:p {:rkey :p1} "Paragraph 1"]
                          [:p {:rkey :p2} "Paragraph 2"]])
               (h/render [:div
                          [:h1 {} "Title"]
                          [:p {:rkey :p0} "Paragraph 0"]
                          [:p {:rkey :p1} "Paragraph 1"]
                          [:p {:rkey :p2} "Paragraph 2"]])
               h/get-mutation-log-events
               h/summarize)
  ...
  (testing "Triggers on-update on moving children"
    (is (= (let [res (atom [])
             ...
             (-> (h/render [:div
                            ...
                            [:p.p1 {:rkey :p1 :replicant/on-update f} "Two"]
                            [:p.p2 {:rkey :p2 :replicant/on-update f} "Three"]
                            [:p.p3 {:rkey :p3 :replicant/on-update f} "Four"]
                            [:p.p4 {:rkey :p4 :replicant/on-update f} "Five"]])
                 (h/render [:div
                            [:h1 {:replicant/on-update f} "One"]
                            [:p.p2 {:rkey :p2 :replicant/on-update f} "Three"]
                            [:p.p3 {:rkey :p3 :replicant/on-update f} "Four"]
